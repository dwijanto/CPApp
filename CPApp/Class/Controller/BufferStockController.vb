Public Class BufferStockController
    Implements IController
    Implements IToolbarAction

    Public Model As New BufferStockModel
    ...
    Public Function GetDSForImport() As DataSet
        Dim myret As Boolean = False
        Model = New BufferStockModel
        DS = New DataSet
        If Model.LoadData(DS, "") Then
            Dim pk(2) As DataColumn
            pk(0) = DS.Tables(0).Columns("customercode")
            pk(1) = DS.Tables(0).Columns("vendorcode")
            pk(2) = DS.Tables(0).Columns("partno")
            DS.Tables(0).PrimaryKey = pk
            DS.Tables(0).Columns("id").AutoIncrement = True
            DS.Tables(0).Columns("id").AutoIncrementSeed = -1
            DS.Tables(0).Columns("id").AutoIncrementStep = -1
            Dim pk1(0) As DataColumn
            pk1(0) = DS.Tables(1).Columns("customercode")
            DS.Tables(1).PrimaryKey = pk1
            Dim pk2(0) As DataColumn
            pk2(0) = DS.Tables(2).Columns("vendorcode")
